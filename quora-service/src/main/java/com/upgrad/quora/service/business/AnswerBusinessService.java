...
public class AnswerBusinessService {
    ...
    public AnswerEntity createAnswer(AnswerEntity answerEntity, final String questionId, final String authorizationToken)
    ...
    @Transactional(propagation = Propagation.REQUIRED)
    public AnswerEntity editAnswerContent(AnswerEntity answerEntity, final String answerId, final String authorizationToken)
        ...
        if (!userAuthTokenEntity.getUser().getId().equals(answerEntity1.getUsers().getId())){
    ...
    public AnswerEntity deleteAnswer(final String answerId, final String authorizationToken)
        ...
        Integer loggedInUserId = userAuthTokenEntity.getUser().getId();
        Integer answerUserId = answerEntity.getUsers().getId();

        if (!(userRole.equals("admin")) && (loggedInUserId != answerUserId)) {
    ...
    @Transactional(propagation = Propagation.REQUIRED)
    public List<AnswerEntity> getAllAnswersToQuestion(final String questionId, final String authorizationToken)
            throws AuthorizationFailedException, InvalidQuestionException {
        ...
        List<AnswerEntity> answerEntityList = answerDao.getAllAnswersToQuestion(questionEntity);
